//
//  HTCustomerFaceVipTableViewCell.m
//  YS_zhtx
//
...
#import "MJPhoto.h"
@interface HTCustomerFaceVipTableViewCell()
...
@end
@implementation HTCustomerFaceVipTableViewCell
...
- (void)awakeFromNib {
    ...
    [self.currentImg changeCornerRadiusWithRadius:self.currentImg.height/2];
    
}

- (void)setVipModel:(HTNewFaceVipModel *)vipModel{
    _vipModel = vipModel;
    ...
    self.levelLabel.text = vipModel.level;
    //这个if是是否已购
    if (0) {
...
- (IBAction)receptionBtClicked:(id)sender {
    ...
    if ([bt.titleLabel.text isEqualToString:@"接待"]) {
        if (self.delegate ) {
            [self.delegate receptionCustmerVipWithCell:self];
        }
    }else if ([bt.titleLabel.text isEqualToString:@"再购"]){
        if (self.delegate ) {
            [self.delegate repitBuyVipWithCell:self];
        }
    }else if ([bt.titleLabel.text isEqualToString:@"录入资料"]){
        if (self.delegate ) {
            [self.delegate writeCutomerInfoVipWithCell:self];
        }
    }
}
