...
@interface HTNewPayViewController ()<UITableViewDelegate, UITableViewDataSource, UITextViewDelegate, paypayNormalDelegate, chooseSellerBackDelegate, UITextFieldDelegate, MFMessageComposeViewControllerDelegate, HTYHQListChooseDelegate>{
    ...
    NSString *yhqText;
    HTYHQModel *yhqModel;
    __weak IBOutlet UITextField *maskScoreTF;
    __weak IBOutlet UITextField *maskMoneyTF;
...
- (void)selectMixSell{
    ...
    if (firstResponder.tag >= 4000 && firstResponder.tag < 5000) {
        firstResponder.text = [NSString stringWithFormat:@"%.2f", _mixRemainSellNum / [_orderModel.encodeFinal floatValue] * 100];
    }else{
...
- (void)dealSellerData{
    ...
    CGFloat totalPrice = [_orderModel.encodeFinal floatValue];
...
- (void)creatBottomView{
    ...
    self.priceLabel.text = [NSString stringWithFormat:@"¥%@", [HTHoldNullObj getValueWithUnCheakValue:_orderModel.encodeFinal]];
    //中划线
...
- (IBAction)sureToPayAction:(id)sender {
    if (!isNormalPayKind) {
        ...
        if (fabsf(total - [_orderModel.encodeFinal floatValue]) >= 0.01) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        ...
        cell.priceLabel.text = [NSString stringWithFormat:@"¥%@", [HTHoldNullObj getValueWithUnCheakValue:_orderModel.encodeFinal]];
        ...
        if (_custModel.phone == nil || [_custModel.phone isEqualToString:@""]) {
            cell.hidden = YES;
        }
        return cell;
...
{
    if (model.cardTemplateId == 0) {
        yhqModel = nil;
        yhqText = @"无";
    }else{
        yhqModel = model;
        yhqText = yhqModel.name;
    }
    [_dataTableView reloadData];
}
...
- (BOOL)checkPayIfCanThisEditWithTextField:(UITextField *)textField WithStr:(NSString *)str{
    ...
    if (tag >= 3000 && tag < 4000) {
        ...
        if ([_orderModel.encodeFinal floatValue] >= totalNum) {
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 1) {
        ...
        vc.didChange = ^(NSArray *backArray, NSString *wechatPayOrderId, NSString *finalPrice, NSString *payCode) {
            ...
            strongSelf.orderModel.encodeFinal = finalPrice;
        ...
        if (indexPath.row == 0) {
            ...
            vc.delegate = self;
            if (yhqModel) {
                vc.currentModel = yhqModel;
            }
            if (_custModel.phone == nil || [_custModel.phone isEqualToString:@""]) {
                vc.customerId = @"";
                vc.phone = @"";
            }else{
                vc.customerId = [HTHoldNullObj getValueWithUnCheakValue: _custModel.custId];
                vc.phone = _custModel.phone;
            }
            vc.companyId = [HTHoldNullObj getValueWithUnCheakValue:[HTShareClass shareClass].loginModel.companyId];
            [self.navigationController pushViewController:vc animated:YES];
        }
    } else if (indexPath.section == 6) {
...
- (IBAction)maskCancelAction:(id)sender {
    NSLog(@"取消按钮");
    maskScoreTF.text = @"";
    maskMoneyTF.text = @"";
    maskView.hidden = YES;
}
- (IBAction)maskSureAction:(id)sender {
    NSLog(@"确认按钮");
    [self getFinalPrice];
    maskView.hidden = YES;
