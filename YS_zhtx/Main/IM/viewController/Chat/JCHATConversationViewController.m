...
- (void)viewDidLoad {
    ...
    //(@"Action - viewDidLoad");
    if (_conversation.title != self.chatName && self.chatName.length > 0) {
...
- (void)viewWillAppear:(BOOL)animated {
  ...
  [self.toolBarContainer.toolbar drawRect:self.toolBarContainer.toolbar.frame];
    [HTShareClass shareClass].msgConversation = _conversation;
    kWEAKSELF
    [_conversation refreshTargetInfoFromServer:^(id resultObject, NSError *error) {
        //(@"refresh nav right button");
        kSTRONGSELF
...
//
//    NSArray *arr =   [_conversation messageArrayFromNewestWithOffset:nil limit:@(10)];
//    for (JMSGMessage *msg in arr) {
//        if (msg.isReceived) {
//            message = msg;
//        }
//    }
//    CGFloat nowTime = [[NSDate date] timeIntervalSince1970] * 1000;
//    NSNumber *titmesTap = message.timestamp;
//
//    if ((nowTime - titmesTap.floatValue) / (3600 * 1000) < 48) {
//        NSDictionary *extras = [[message.originMeta getDictionArrayWithKey:@"msg_body"] getDictionArrayWithKey:@"extras"];
//        if ([extras getStringWithKey:@"customerId"].length > 0) {
//            self.extras = extras;
//        }else{
//            self.extras = nil;
//        }
...
- (void)queryUserState{
    ...
    [HTHttpTools POST:[NSString stringWithFormat:@"%@%@",baseUrl,@"wechat_mp/query_user_state.html"] params:dic success:^(id json) {
        
        if([[json getStringWithKey:@"state"] isEqualToString:@"1"]){
        NSDictionary *dic = @{
                              @"openId":[json getStringWithKey:@"openid"]
                              };
        self.extras = dic;
        }
    } error:^{
...
- (void)viewDidDisappear:(BOOL)animated{
    ...
    [HTShareClass shareClass].msgConversation = nil;
}
...
- (void)onSendMessageResponse:(JMSGMessage *)message error:(NSError *)error {
  ...
  if (message != nil) {
    ...
    if ([message.content isKindOfClass:[JMSGMediaAbstractContent class]] && [self.msgId isEqualToString:message.msgId]) {
        JMSGMediaAbstractContent *coent = (id)message.content;
        NSDictionary *dic = @{
                              @"companyId":[HTShareClass shareClass].loginModel.companyId,
                              @"customerId":[HTHoldNullObj getValueWithUnCheakValue:self.modelId],
                              @"openId":[HTHoldNullObj getValueWithUnCheakValue:[self.extras getStringWithKey:@"openId"] ],
                              @"mediaId":[HTHoldNullObj getValueWithUnCheakValue:coent.mediaID]
                              };
        [HTHttpTools POST:[NSString stringWithFormat:@"%@%@",baseUrl,@"wechat_mp/send_kefu_msg.html"] params:dic success:^(id json) {
            self.msgId = @"";
        } error:^{
...
- (void)onSyncOfflineMessageConversation:(JMSGConversation *)conversation
    ...
    //(@"Action -- onSyncOfflineMessageConversation:offlineMessages:");
...
- (void)relayoutTableCellWithMessage:(JMSGMessage *) message{
    //(@"relayoutTableCellWithMessage: msgid:%@",message.msgId);
...
- (void)addMessage:(JCHATChatModel *)model {
  if (model.isTime) {
    ...
    if (model.message.msgId.length == 0 ) {
        [MBProgressHUD showMessage:@"正在重新登录聊天" toView:self.view];
        [JMSGUser loginWithUsername:[HTShareClass shareClass].JMessageUserId password:[HTShareClass shareClass].JMessagePwd completionHandler:^(id resultObject, NSError *error) {
            if (!error) {
...
//  [lib enumerateGroupsWithTypes:ALAssetsGroupSavedPhotos usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
    UIImagePickerController *photoPickerVC = [[UIImagePickerController alloc] init];
    photoPickerVC.delegate = self;
    photoPickerVC.navigationBar.translucent = NO;
...
- (void)prepareImageMessage:(UIImage *)img {
  //(@"Action - prepareImageMessage");
    
    if (self.extras.allKeys.count > 0) {
        ...
        if (!isSend) {
            NSDictionary *dic = @{
                                  @"companyId":[HTShareClass shareClass].loginModel.companyId,
                                  @"cutomerId":[HTHoldNullObj getValueWithUnCheakValue:self.modelId],
                                  
                                  };
            [HTHttpTools POST:[NSString stringWithFormat:@"%@%@",baseUrl,@"wechat_mp/sendMessage.html"] params:dic success:^(id json) {
            } error:^{
                [MBProgressHUD showError:SeverERRORSTRING toView:self.view];
            } failure:^(NSError *error) {
                [MBProgressHUD showError:@"请检查你的网络" toView:self.view];
...
- (void)prepareTextMessage:(NSString *)text {
    ...
    if (self.extras.allKeys.count > 0) {
        NSDictionary *dic = @{
                              ...
                              @"customerId":[HTHoldNullObj getValueWithUnCheakValue:self.modelId],
                              @"openId":[HTHoldNullObj getValueWithUnCheakValue:[self.extras getStringWithKey:@"openId"] ],
                              @"msg":[HTHoldNullObj getValueWithUnCheakValue:text]
                              };
        [HTHttpTools POST:[NSString stringWithFormat:@"%@%@",baseUrl,@"wechat_mp/send_kefu_msg.html"] params:dic success:^(id json) {
        } error:^{
            [MBProgressHUD showError:SeverERRORSTRING toView:self.view];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
  if (!isNoOtherMessage) {
    ...
    if (indexPath.row >= _allmessageIdArr.count) {
        //(@"1.index %ld beyond bounds %ld",indexPath.row,_allmessageIdArr.count);
        return 40;
    }
...
//          [self performSelector:@selector(flashToLoadMessage) withObject:nil afterDelay:0];
    ...
    if (indexPath.row >= _allmessageIdArr.count) {
        //(@"2.index %ld beyond bounds %ld",indexPath.row,_allmessageIdArr.count);
        return nil;
    }
    NSString *messageId = _allmessageIdArr[indexPath.row];
    if (!messageId) {
        //(@"messageId is nil%@",messageId);
        return nil;
    }

    JCHATChatModel *model = _allMessageDic[messageId];
    if (!model) {
        //(@"JCHATChatModel is nil%@", messageId);
        return nil;
    }
...
- (void)refreshCellMessageMediaWithChatModel:(JCHATChatModel *)model {
    //(@"Action - refreshCellMessageMediaWithChatModel:");
...
- (void)chcekReceiveMessageAvatarWithReceiveNewMessage:(JMSGMessage *)message {
    //(@"chcekReceiveMessageAvatarWithReceiveNewMessage:%@",message.serverMessageId);
...
- (void)refreshVisibleRowsAvatarWithNewMessage:(JMSGMessage *)message avatarDataLength:(NSUInteger)length {
    ...
    //(@"refreshVisibleRowsAvatarWithNewMessage::%@",message.serverMessageId);
...
- (void)reloadAllCellAvatarImage {
    //(@"Action -reloadAllCellAvatarImage");
...
- (void)selectHeadView:(JCHATChatModel *)model {
  ...
  if (![model.message isReceived]) {
      ...
      vc.guiderId = [HTShareClass shareClass].loginId;
      vc.title = [HTShareClass shareClass].loginModel.person.name;
...
//  //(@"Action - didStartRecordingVoice");
  [self startRecord];
}

- (void)didCancelRecordingVoiceAction {
//  //(@"Action - didCancelRecordingVoice");
  [self cancelRecord];
}

- (void)didFinishRecordingVoiceAction {
//  //(@"Action - didFinishRecordingVoiceAction");
  [self finishRecorded];
}

- (void)didDragOutsideAction {
  //(@"Action - didDragOutsideAction");
  [self resumeRecord];
}

- (void)didDragInsideAction {
  //(@"Action - didDragInsideAction");
  [self pauseRecord];
}
