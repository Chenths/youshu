            ...
            [group enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
              if (result != nil) {
                ...
                [model setDatawithAsset:result];
                  [self->allPhotoArr addObject:model];
              }
            }];
            // 将所有获取的 照片模型 交给manager统一管理
              [self->photoGridView reloadData];
              [self->photoGridView scrollToItemAtIndexPath:[NSIndexPath indexPathForItem:self->allPhotoArr.count - 1 inSection:0] atScrollPosition:UICollectionViewScrollPositionNone animated:NO];
          }
