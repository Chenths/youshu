    ...
    for (int i = 0; i < count; i++) {
        ...
        if (key && value) {
            if ([value isKindOfClass:[NSString class]]
                ...
                // 普通类型的直接变成字典的值
                [dict setObject:[value isNull] ? @"" : value forKey:key];
            }
...
//            [dict setObject:[value isKindOfClass:[NSDictionary class]] ? [NSDictionary dictionary] : [value isKindOfClass:[NSArray class]] ? [NSArray array] : @"" forKey:key];
        }
    }
