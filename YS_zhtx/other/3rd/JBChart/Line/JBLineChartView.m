...
- (void)reloadDataAnimated:(BOOL)animated
{
	...
	self.reloading = YES;
    [backLinesView removeFromSuperview];
    backLinesView = [[UIView alloc] initWithFrame:self.bounds];
    CGFloat space =  (self.height - 20) / 6;
    for (int i = 0 ; i < 6; i++) {
        UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 4 + space *i, self.width, 0.5)];
        line.backgroundColor = [UIColor colorWithHexString:@"#dddddd"];
        [backLinesView addSubview:line];
    }
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, [self height] - 20, self.width,1)];
    line.backgroundColor = [UIColor colorWithHexString:@"#222222"];
    [backLinesView addSubview:line];
	...
	dispatch_block_t createChartDataBlock = ^{
		...
		for (NSUInteger lineIndex=0; lineIndex<numberOfLines; lineIndex++)
		{
			...
			for (NSUInteger horizontalIndex=0; horizontalIndex<dataCount; horizontalIndex++)
			{
                ...
                UIView *line = [[UIView alloc] initWithFrame:CGRectMake(chartPointModel.position.x,0, 0.5, self.height - 20)];
                line.backgroundColor = [UIColor colorWithHexString:@"#dddddd"];
                [backLinesView addSubview:line];
                NSString *countData = [self.delegate lineChartView:self verticalXValueForHorizontalIndex:horizontalIndex atLineIndex:lineIndex];
                UILabel *countLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, [countData getStringWidhtWithHeight:15 andFont:11], 15)];
                countLabel.textColor = [UIColor colorWithHexString:@"222222"];
                countLabel.text = countData;
                countLabel.textAlignment = NSTextAlignmentCenter;
                countLabel.font = [UIFont systemFontOfSize:11];
                CGFloat x = horizontalIndex == 0 ? countLabel.width / 2 : ((horizontalIndex == dataCount - 1 ) ? (chartPointModel.position.x - countLabel.width / 2) : chartPointModel.position.x);
                countLabel.center = CGPointMake( x , [self height] - 7.5);
                [backLinesView addSubview:countLabel];
                [lineChartLine.lineDatas addObject:@([self.delegate lineChartView:self verticalValueForHorizontalIndex:horizontalIndex atLineIndex:lineIndex])];
				xOffset += pointSpace;
			}
	...
	dispatch_block_t createSelectionViewBlock = ^{
		...
		CGFloat verticalSelectionViewHeight = self.bounds.size.height;
...
//                verticalSelectionViewHeight += self.footerPadding;
    ...
    [self addSubview:backLinesView];
    [self insertSubview:backLinesView atIndex:0];
}
...
- (CGFloat)availableHeight
{
	return self.bounds.size.height - self.headerView.frame.size.height - self.footerView.frame.size.height - self.headerPadding - self.footerPadding - 20;
}
