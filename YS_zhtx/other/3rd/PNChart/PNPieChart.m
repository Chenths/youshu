...
- (void)loadDefault{
    ...
    [_items enumerateObjectsUsingBlock:^(PNPieChartDataItem *item, NSUInteger idx, BOOL *stop) {
        if (total == 0){
            [endPercentages addObject:@(1.0 / self->_items.count * (idx + 1))];
        }else{
    ...
    [descriptionLabel1 removeFromSuperview];
...
- (void)recompute {
    if (self.innerCircleRadius != CGRectGetWidth(self.bounds) / 6) {
        return;
    }
...
- (void)setPieTitelText:(NSString *)pieTitelText{
    _pieTitelText = pieTitelText;
    }
- (void)strokeChart{
    ...
    descriptionLabel1 = [[UILabel alloc] initWithFrame:CGRectMake(_contentView.center.x - _innerCircleRadius + 5 , _contentView.center.y - _innerCircleRadius - 5 ,_innerCircleRadius*2 , 50)];
    descriptionLabel1.center = CGPointMake(_contentView.center.x , _contentView.center.y );
    NSString *titleText = self.pieTitelText;
    descriptionLabel1.font = [UIFont systemFontOfSize:16];
    descriptionLabel1.text = titleText;
...
- (void)addAnimationIfNeeded{
    if (self.displayAnimated) {
        ...
        [_descriptionLabels enumerateObjectsUsingBlock:^(UIView *obj, NSUInteger idx, BOOL *stop) {
            [obj setAlpha:1];
        }];
    }
}
...
- (void)didTouchAt:(CGPoint)touchLocation
{
    ...
    if ([self.delegate respondsToSelector:@selector(userClickedOnPieIndexItem:onPie:)]  ) {
        [self.delegate userClickedOnPieIndexItem:index onPie:self];
    }
...
- (UIView*) getLegendWithMaxWidth:(CGFloat)mWidth{
    ...
    for (PNPieChartDataItem *pdata in self.items) {
        ...
        NSMutableAttributedString *mabstring = [[NSMutableAttributedString alloc] initWithString:[HTHoldNullObj getValueWithUnCheakValue:pdata.textDescription]];
        
        NSString *dataStr = [HTHoldNullObj getValueWithUnCheakValue:pdata.textDescription];
       
        NSArray *subStrs = [dataStr componentsSeparatedByString:@"("];
        NSString *lastStr = subStrs.lastObject;
        if (lastStr.length > 2) {
            NSString *lastTwo = [lastStr substringWithRange:NSMakeRange(lastStr.length - 2, 2)];
            if ([lastTwo isEqualToString:@"%)"]) {
                NSRange range = [dataStr rangeOfString:[NSString stringWithFormat:@"(%@",lastStr]];
                
                if (range.length != 0 ) {
                    [mabstring addAttribute:NSForegroundColorAttributeName
                                      value:[UIColor colorWithRed:234 / 255.0 green:65 / 255.0 blue:60 / 255.0 alpha:1]
                                      range:range];
            ...
            label.text = [HTHoldNullObj getValueWithUnCheakValue:pdata.textDescription];
            label.textColor = self.legendFontColor ? self.legendFontColor : [UIColor blackColor];
        }
