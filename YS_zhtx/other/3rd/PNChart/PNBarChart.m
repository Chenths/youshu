...
- (void)setXLabels:(NSArray *)xLabels
{
    ...
    _xLabelWidth = (self.frame.size.width - _chartMarginLeft - _chartMarginRight) /   4;
    if (_showLabel) {
        int labelAddCount = 0;
        for (int index = 0; index < _xLabels.count; index++) {
            labelAddCount += 1;
            ...
            if (labelAddCount == _xLabelSkip) {
                ...
                label.center = CGPointMake(labelXPosition - 0.5 * (_xLabelWidth - _barWidth) ,
                                           self.frame.size.height - kXLabelHeight - _chartMarginTop + label.frame.size.height / 2.0 + _labelMarginTop - 20);
                labelAddCount = 0;
...
- (void)strokeChart
{
    ...
    if (_showChartBorder) {
        ...
        [progressline moveToPoint:CGPointMake(0, self.frame.size.height - kXLabelHeight - _chartMarginBottom + _chartMarginTop)];
        [progressline addLineToPoint:CGPointMake(self.frame.size.width ,  self.frame.size.height - kXLabelHeight - _chartMarginBottom + _chartMarginTop)];
...
- (void)touchPoint:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([subview isKindOfClass:[PNBar class]] && [self.delegate respondsToSelector:@selector(userClickedOnBarAtIndex: onBar:)]) {
        [self.delegate userClickedOnBarAtIndex:subview.tag onBar:self];
    }
}
