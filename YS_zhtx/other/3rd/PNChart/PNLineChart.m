...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(touchbegin)]) {
...
- (void) doSomeThing{
    if (isEnd) {
        if (_delegate && [_delegate respondsToSelector:@selector(chancelDoSomething)]) {
            [_delegate chancelDoSomething];
        }
    }
}
            ...
            if (distance <= 5.0) {
                ...
                for (UIBezierPath *path in _chartPath) {
                    ...
                    if (pointContainsPath) {
                        ...
                        NSArray *linePointsArray1 = _pathPoints[[_chartPath indexOfObject:path]];
                        
                        for (int i = 0; i < (int) linePointsArray1.count - 1; i += 1) {
                            CGPoint p1 = [linePointsArray1[i] CGPointValue];
                            CGPoint p2 = [linePointsArray1[i + 1] CGPointValue];
                            
                            float distanceToP1 = fabs(touchPoint.x - p1.x);
                            float distanceToP2 = fabs(touchPoint.x - p2.x);
...
- (void)strokeChart {
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            int x = _chartMarginLeft + (i * _xLabelWidth) +  _xLabelWidth / 2.0;
            int y =  _chartMarginBottom + _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2) - _chartMarginTop;
            NSString *string = [NSString stringWithFormat:@"%.0lf",yValue];
            float  width = [string boundingRectWithSize:CGSizeMake(MAXFLOAT, 50) options:0 attributes:@{NSFontAttributeName :[UIFont systemFontOfSize:14]} context:nil].size.width + 10;
            
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(x - width / 2, y + 1 - 20 , width, 14)];
...
//            int x = i * _xLabelWidth + _chartMarginLeft + _xLabelWidth / 2.0;
            int x = _chartMarginLeft + (i * _xLabelWidth) +  _xLabelWidth / 2.0;
            int y =  _chartMarginBottom + _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2) - _chartMarginTop;
//            _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2) + _chartMarginTop - _chartMarginBottom;

            NSString *string = [NSString stringWithFormat:@"%.2lf",yValue];
            float  width = [string boundingRectWithSize:CGSizeMake(MAXFLOAT, 50) options:0 attributes:@{NSFontAttributeName :[UIFont systemFontOfSize:14]} context:nil].size.width + 10;
            
            UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(x - width / 2, y + 1, width, 15)];
            [self addSubview:label];
    ...
    if (self.isShowCoordinateAxis) {
        ...
        if (self.showLabel) {
            ...
            for (NSUInteger i = 0; i < [self.xLabels count]; i++) {
                ...
                point = CGPointMake(_chartMarginLeft + (i * _xLabelWidth) +  _xLabelWidth / 2.0, _chartMarginBottom + _chartCavanHeight);
...
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
        ...
        size = [[HTHoldNullObj getValueWithUnCheakValue:text] sizeWithFont:font constrainedToSize:size lineBreakMode:NSLineBreakByCharWrapping];
