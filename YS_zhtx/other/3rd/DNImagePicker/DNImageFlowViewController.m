...
- (BOOL)seletedAssets:(ALAsset *)asset
{
    ...
    if (self.selectedAssetsArray.count >= [HTShareClass shareClass].seltedPhotosCount) {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedStringFromTable(@"alertTitle", @"DNImagePicker", nil) message:[NSString stringWithFormat:@"不能超过%ld张图片",(unsigned long)[HTShareClass shareClass].seltedPhotosCount] delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"alertButton", @"DNImagePicker", nil) otherButtonTitles:nil, nil];
        [alert show];
...
- (UICollectionView *)imageFlowCollectionView
{
    if (nil == _imageFlowCollectionView) {
        ...
        _imageFlowCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - nav_height - tar_height ) collectionViewLayout:layout];
