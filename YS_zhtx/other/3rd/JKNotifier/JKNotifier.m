...
- (UIImage*)defaultIcon{
    if (!_defaultIcon) {
        ...
        _defaultIcon = [self loadPlistIcon] ?:[UIImage imageNamed:@"AppIcon"] ?:[UIImage imageNamed:@"AppIcon-1"] ?:[UIImage imageNamed:@"AppIcon-2"] ?:[UIImage imageNamed:@"AppIcon-3"] ?:[UIImage imageNamed:@"logo"];
    }
    return _defaultIcon;
}
...
+ (JKNotifierBar*)showNotifer:(NSString*)note name:(NSString*)appName icon:(UIImage*)appIcon{
    return [JKNotifier showNotifer:note name:appName icon:appIcon dismissAfter:5];
}
...
+ (JKNotifierBar*)showNotifer:(NSString*)note
    ...
    UISwipeGestureRecognizer   *swipeGestureRecognizer=[[UISwipeGestureRecognizer alloc]initWithTarget:self action:@selector(dismiss)];
    [swipeGestureRecognizer setDirection:UISwipeGestureRecognizerDirectionUp];
    
    [bar addGestureRecognizer:swipeGestureRecognizer];
