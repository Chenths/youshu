...
- (void)appendString:(NSString *)aString{
    [self appendText:aString alignment:HLTextAlignmentLeft];
}
/**
 *  换行
 */
...
- (void)appendImage:(UIImage *)image alignment:(HLTextAlignment)alignment maxWidth:(CGFloat)maxWidth
{
    ...
    NSData *fData = UIImageJPEGRepresentation(newImage, 0.5);
    UIImage *imgg =   [UIImage imageWithData:fData];
    
    NSData *imageData = [imgg bitmapData];
...
{
    [self appendBarCodeWithInfo:info alignment:HLTextAlignmentCenter maxWidth:200];
}
...
{
    [self appendQRCodeWithInfo:info centerImage:nil alignment:HLTextAlignmentCenter maxWidth:360];
}
